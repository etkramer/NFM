		...
		public void Execute(GeneratorExecutionContext context)
		{
			...
			foreach (SyntaxTree syntaxTree in context.Compilation.SyntaxTrees)
			{
				...
				foreach (AttributeSyntax attributeSyntax in syntaxTree.GetRoot().DescendantNodesAndSelf().OfType<AttributeSyntax>())
				{
					...
					if (attributeType.GetFullName() == "NFM.Aspects.DeclarativePropertyAttribute")
					{
						if (!types.ContainsKey(declaringType))
						{
							AttributeData[] data = declaringType.GetAttributes().Where(o => o.AttributeClass.GetFullName() == "NFM.Aspects.DeclarativePropertyAttribute")?.ToArray();
							if (data != null)
							{
